...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b px-4 sm:px-6 py-4 flex justify-between items-center">
      ...
      {/* LOGO */}
      <Link href="/">
        <img src={logo} alt="ALPS Logo" className="h-8 sm:h-10 cursor-pointer object-contain" />
      </Link>

      {/* ---------- DESKTOP NAV ---------- */}
      <div className="hidden md:flex gap-8 text-sm font-semibold text-primary items-center mx-2">
        <Link href="/" className="hover:text-accent border-b-2 border-transparent hover:border-accent pb-1">
          Home
        </Link>
      ...
      {isMobileMenuOpen && (
        <div className="absolute top-full left-0 right-0 bg-white border-b shadow-2xl md:hidden animate-in slide-in-from-top duration-300">
          <div className="flex flex-col p-6 gap-4">
            ...
            <Link href="/" onClick={() => setIsMobileMenuOpen(false)} className="text-lg font-bold border-b pb-2">
              Home
            </Link>
